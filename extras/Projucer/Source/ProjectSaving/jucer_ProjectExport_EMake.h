...
//==============================================================================
class EMakefileProjectExporter final : public ProjectExporter
{
protected:
    //==============================================================================
    class EMakeBuildConfiguration final : public BuildConfiguration
    {
    public:
        EMakeBuildConfiguration (Project& p, const ValueTree& settings, const ProjectExporter& e)
    ...
    {
        return *new EMakeBuildConfiguration (project, tree, *this);
    }

public:
    //==============================================================================
    class EMakefileTarget final : public build_tools::ProjectType::Target
    {
    public:
        EMakefileTarget (build_tools::ProjectType::Target::Type targetType, const EMakefileProjectExporter& exporter)
        ...
        StringArray getTargetSettings (const EMakeBuildConfiguration& config) const
        {
        ...
        const EMakefileProjectExporter& owner;
    };

    //==============================================================================
    static String getDisplayName()        { return "Emscripten Makefile"; }
    static String getValueTreeTypeName()  { return "EMSCRIPTEN_MAKE"; }
    static String getTargetFolderName()   { return "Emscripten"; }

    Identifier getExporterIdentifier() const override { return getValueTreeTypeName(); }

    static EMakefileProjectExporter* createForSettings (Project& projectToUse, const ValueTree& settingsToUse)
    {
        if (settingsToUse.hasType (getValueTreeTypeName()))
            return new EMakefileProjectExporter (projectToUse, settingsToUse);

        return nullptr;
    }

    //==============================================================================
    EMakefileProjectExporter (Project& p, const ValueTree& t)
        : ProjectExporter (p, t),
        ...
        callForAllSupportedTargets ([this] (build_tools::ProjectType::Target::Type targetType)
                                    {
                                        targets.insert (targetType == build_tools::ProjectType::Target::AggregateTarget ? 0 : -1,
                                                        new EMakefileTarget (targetType, *this));
                                    });
...
private:
    ...
    //==============================================================================
    void writeDefineFlags (OutputStream& out, const EMakeBuildConfiguration& config) const
    {
    ...
    void writeCppFlags (OutputStream& out, const EMakeBuildConfiguration& config) const
    {
    ...
    void writeConfig (OutputStream& out, const EMakeBuildConfiguration& config) const
    {
    ...
    */
    ...
    void writeMakefile (OutputStream& out) const
    {
        ...
        for (ConstConfigIterator config (*this); config.next();)
            writeConfig (out, dynamic_cast<const EMakeBuildConfiguration&> (*config));
        ...
        const auto getFilesForTarget = [this] (const std::vector<std::pair<File, String>>& files,
                                               EMakefileTarget* target,
                                               const Project& p)
        {
            std::vector<std::pair<build_tools::RelativePath, String>> targetFiles;

            auto targetType = (p.isAudioPluginProject() ? target->type : EMakefileTarget::SharedCodeTarget);
            ...
            if ((      targetType == EMakefileTarget::SharedCodeTarget
                    || targetType == EMakefileTarget::StaticLibrary
                    || targetType == EMakefileTarget::DynamicLibrary)
            ...
            if (targetType == EMakefileTarget::LV2Helper)
            {
            ...
            else if (targetType == EMakefileTarget::VST3Helper)
            {
    ...
    {
        if (auto* makeConfig = dynamic_cast<const EMakeBuildConfiguration*> (&config))
    ...
    OwnedArray<EMakefileTarget> targets;

    JUCE_DECLARE_NON_COPYABLE (EMakefileProjectExporter)
};
